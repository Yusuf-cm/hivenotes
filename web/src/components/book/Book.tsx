...
export default function Book({
  ...
  getText, getInk, updateText, updateInk, ensurePage, canEdit, bookOwnerName,
  noteActions, onAddNote, onAddFile,
  ...
  const goNext = useCallback(() => {
    if (flip) return
    const target = nextIndex ?? (canEdit && ensurePage ? pi + 1 : undefined)
    if (target === undefined) return
    if (nextIndex === undefined && ensurePage) ensurePage(target)
    setFlip('next')
    setTimeout(() => { goTo(target); setFlip(null) }, 750)
  }, [flip, nextIndex, goTo, canEdit, ensurePage, pi])

  const goPrev = useCallback(() => {
    if (flip || prevIndex === undefined) return
  ...
  const pageObj = (idx: number) => {
    const existing = pages.find(p => p.pageIndex === idx)
    return {
      id: existing?.id || '',
      roomId: user.roomId,
      pageIndex: idx,
      text: getText(idx),
      ink: getInk ? getInk(idx) : (existing?.ink || []),
      diagram: existing?.diagram || { elements: [] },
      ...
      {isOpen && (
        <div style={{
          ...
          <button
            className="liq"
            onClick={goPrev}
            disabled={prevIndex === undefined || !!flip}
            style={{
              width: 36, height: 36, borderRadius: '50%',
              fontSize: 22,
              color: prevIndex === undefined ? 'rgba(245,237,216,0.22)' : 'var(--cream)',
          ...
          <button
            className="liq"
            onClick={goNext}
            disabled={!!flip || (nextIndex === undefined && !canEdit)}
            style={{
              width: 36, height: 36, borderRadius: '50%',
              fontSize: 22,
              color: (nextIndex === undefined && !canEdit) ? 'rgba(245,237,216,0.22)' : 'var(--cream)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
          >›</button>
        </div>
      ...
      >
        ...
        {isOpen && (
          <div style={{
            ...
            {prevIndex !== undefined ? (
              <PageContent
                page={pageObj(prevIndex)}
                notes={notesOn(prevIndex)}
                onTextChange={() => {}}
                onAddNote={() => {}}
                noteActions={noteActions}
                interactive={false}
        ...
        <div style={{
          ...
          <PageContent
            ...
            onInkChange={updateInk ? ink => updateInk(pi, ink) : undefined}
            onAddNote={(x, y) => onAddNote(pi, x, y)}
            onAddFile={onAddFile ? (x, y, file) => onAddFile(pi, x, y, file) : undefined}
            noteActions={noteActions}
            interactive={isOpen && !flip && canEdit}
            writeMode={writeMode}
            inkTool={inkTool}
            inkColor={inkColor}
            inkWidth={inkWidth}
        ...
        {flip && isOpen && (
          <div style={{
            ...
            <div style={{
              ...
              <PageContent
                page={pageObj(flip === 'next' ? pi : (prevIndex ?? pi))}
                notes={notesOn(flip === 'next' ? pi : (prevIndex ?? pi))}
                onTextChange={() => {}}
                onAddNote={() => {}}
                noteActions={noteActions}
                interactive={false}
