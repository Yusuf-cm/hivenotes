...
import { canWriteNote, getRoomPage, isPageOwner } from '../lib/ownedPage'
...
router.get('/page/:pageIndex', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const pageOwnerId = typeof req.query.owner === 'string' ? req.query.owner : req.user!.userId
...
router.post('/', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const targetIndex = typeof pageIndex === 'number' && isFinite(pageIndex) ? pageIndex : 0
    const page = await getRoomPage(req.user!.roomId, req.user!.userId, targetIndex)

    if (!page || !isPageOwner(page, req.user!.userId)) {
      res.status(403).json({ error: 'You can only add notes on your own page' })
      return
    }
...
router.patch('/:id', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const existingPage = await getRoomPage(
      req.user!.roomId,
      existing.pageOwnerId || req.user!.userId,
      existing.pageIndex
    )
    if (!canWriteNote(existing, existingPage, req.user!.userId)) {
      res.status(403).json({ error: 'You can only edit notes you authored or notes on your page' })
      return
    }
    ...
    if (req.body.pageIndex !== undefined) {
      ...
      const targetPage = await getRoomPage(req.user!.roomId, req.user!.userId, req.body.pageIndex)
      if (!targetPage || !isPageOwner(targetPage, req.user!.userId)) {
        res.status(403).json({ error: 'You can only move notes to your own journal' })
        return
      }
      data.pageIndex = req.body.pageIndex
      data.pageOwnerId = req.user!.userId
    }
...
router.delete('/:id', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!canWriteNote(existing, page, req.user!.userId)) {
      res.status(403).json({ error: 'You can only delete notes you authored or notes on your page' })
      return
    }
