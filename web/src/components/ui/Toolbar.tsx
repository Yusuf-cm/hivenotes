...
interface Props {
  ...
  onAI:          () => void
  onLogout:      () => void
  onOpenJournal: (userId: string) => void
  writeMode?:    WriteMode
  onWriteMode?:  (mode: WriteMode) => void
}

export default function Toolbar({
  user, viewingUserId, canEdit, onAddNote, onAudio, onImage, onShare, onAI, onLogout, onOpenJournal,
  writeMode = 'type', onWriteMode,
}: Props) {
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            color: canEdit ? 'var(--cream)' : 'rgba(245,237,216,0.28)',
        ...
        {onWriteMode && (
          <>
            <button
              className="liq"
              onClick={() => onWriteMode(writeMode === 'pen' ? 'type' : 'pen')}
              disabled={!canEdit && writeMode !== 'pen'}
              style={{
                padding: '5px 13px', borderRadius: 22,
                fontFamily: 'var(--font-cormorant)',
                fontSize: 13, letterSpacing: '.08em',
                color: writeMode === 'pen' ? 'var(--gold-lt)' : canEdit ? 'var(--cream)' : 'rgba(245,237,216,0.28)',
                cursor: 'pointer',
              }}
            >
              ✎ Pen
            ...
            <button
              className="liq"
              onClick={() => onWriteMode(writeMode === 'diagram' ? 'type' : 'diagram')}
              disabled={!canEdit && writeMode !== 'diagram'}
              style={{
                padding: '5px 13px', borderRadius: 22,
                fontFamily: 'var(--font-cormorant)',
                fontSize: 13, letterSpacing: '.08em',
                color: writeMode === 'diagram' ? 'var(--gold-lt)' : canEdit ? 'var(--cream)' : 'rgba(245,237,216,0.28)',
                cursor: 'pointer',
              }}
            >
              Diagram
        ...
        <button
          ...
          style={{
            ...
            fontFamily: 'var(--font-cormorant)',
            fontSize: 13, letterSpacing: '.08em',
            color: recording ? 'var(--red)' : canEdit ? 'var(--cream)' : 'rgba(245,237,216,0.28)',
      ...
      {showPresence && presence.length > 1 && (
        ...
        >
          ...
          {presence.map(p => (
            <div
              key={p.userId}
              onClick={() => { onOpenJournal(p.userId); setShowPresence(false) }}
              style={{
              display: 'flex', alignItems: 'center', gap: 9,
              marginBottom: 7,
              cursor: 'pointer',
              opacity: p.userId === viewingUserId ? 1 : 0.85,
            }}>
              <div style={{
